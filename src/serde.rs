...
/// Function to deserializing an integer or a string to an **`integer`**
#[cfg(feature = "num")]
...
impl<'de, T: Default + Float + FromPrimitive + FromStr> de::Visitor<'de> for DeFloatVisitor<T> {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("a float number or a string")
    }
...
/// Function to deserializing a float number or a string to a **`float`**
#[cfg(feature = "num")]
...
/// Function to deserializing a boolean value, an integer (0/1) or
/// a string ("true"/"false") to a **`bool`**
...
/// Function to serializing a **`Vec<String>`** to a simple string with the separator ", ".
...
/// it can recognize separators of ',', ';', or '\n'.
